...
class Model(nn.Module):
    def __init__(self, n_embs, img_embs, n_hidden, vocab_size, dropout, max_len, CLS):
        ...
        self.text_encoder = Transformer(n_embs=n_embs, dim_ff=n_hidden, n_head=8, n_block=1, dropout=dropout)
    ...
    def decode(self, Cap, G, L, mask):
        text_embs = self.word_embedding(Cap)
        M = torch.cat((G, L),dim=1)
        out = self.decoder(text_embs, M, mask)
    ...
    def forward(self, Img1, Img2, Cap):
        ...
        mask = Variable(subsequent_mask(Cap.size(0), Cap.size(1)-1), requires_grad=False).cuda()
        I_img = self.decode(Cap[:,:-1], I, Imgs, mask)
        T_text = self.decode(Cap[:,:-1], I, Text, mask)
