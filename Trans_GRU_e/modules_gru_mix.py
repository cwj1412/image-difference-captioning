...
class Model(nn.Module):
    ...
    def forward(self, Img1, Img2, Cap):
        ...
        outs_it= I_text.transpose(0, 1)
        outs_ti= T_img.transpose(0, 1)

        loss_img = self.criterion(outs_img.contiguous().view(-1, self.vocab_size), Cap[1:].contiguous().view(-1))
        loss_it = self.criterion(outs_it.contiguous().view(-1, self.vocab_size), Cap[1:].contiguous().view(-1))
        loss_ti = self.criterion(outs_ti.contiguous().view(-1, self.vocab_size), Cap[1:].contiguous().view(-1))
        # print(torch.sum(loss_img), torch.sum(loss_text))
        loss = loss_img + loss_it + loss_ti
        #loss = loss_img + loss_ti
        return loss
