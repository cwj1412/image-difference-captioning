...
class Model(nn.Module):
    def __init__(self, n_embs, img_embs, n_hidden, n_head, n_block, vocab_size, dropout, max_len, CLS):
        ...
        self.position_encoding = PositionalEmb(n_hidden, dropout, 2)
        self.img_encoder = Transformer(n_embs=n_embs, dim_ff=n_hidden, n_head=n_head, n_block=4, dropout=dropout)
    ...
    def forward(self, Img1, Img2, Cap):
        ...
        I_text = self.decode(Cap[:,:-1], I, Text)
        T_img = self.decode(Cap[:,:-1], T, Imgs)
        T_text = self.decode(Cap[:,:-1], T, Text)
        
        Cap = Cap.t()
        outs_img = I_img.transpose(0, 1)
        outs_mm1 = I_text.transpose(0, 1)
        outs_mm2 = T_img.transpose(0, 1)
        ...
        loss_mm1 = self.criterion(outs_mm1.contiguous().view(-1, self.vocab_size), Cap[1:].contiguous().view(-1))
        loss_mm2 = self.criterion(outs_mm2.contiguous().view(-1, self.vocab_size), Cap[1:].contiguous().view(-1))
        # print(torch.sum(loss_img), torch.sum(loss_text), torch.sum(loss_mm1), torch.sum(loss_mm2))
        loss = loss_img + loss_text + loss_mm1 + loss_mm2
        return loss
...
class GRU_Decoder(nn.Module):
    ...
    def step_decoding(self, x, L, h):
        ...
        h = torch.cat([h1.unsqueeze(0),h2.unsqueeze(0),h3.unsqueeze(0),h4.unsqueeze(0)],dim=0,out=None) # [layers_num = 8, B, hidden_size]
        #h1 = self.drop(h1)
        return h
