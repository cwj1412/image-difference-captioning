...
class Model(nn.Module):
    def __init__(self, n_embs, img_embs, n_hidden, vocab_size, dropout, max_len):
        ...
        self.img_encoder = Transformer(n_embs=n_embs, dim_ff=n_hidden, n_head=8, n_block=4, dropout=dropout)
        self.decoder = TransformerDecoder(dim=self.n_hidden, dim_ff=self.n_hidden, n_head=8, n_block=4, dropout=dropout)
    ...
    def encode_img(self, img1, img2):
        ...
        H = self.img_encoder(Img)
        return H

    def decode(self, Des, M, mask):
    ...
    def forward(self, Img1, Img2, Des):
        ...
        # Img_G, Imgs = self.encode_img(Img1, Img2)
        Imgs = self.encode_img(Img1, Img2)
        mask = Variable(subsequent_mask(Des.size(0), Des.size(1)-1), requires_grad=False).cuda()

        # T_img = self.decode(Des[:,:-1], Img_G, Imgs, mask=mask)
        T_img = self.decode(Des[:,:-1], Imgs, mask=mask)

        Des = Des.t()
        outs_img = T_img.transpose(0, 1)
        loss = self.criterion(outs_img.contiguous().view(-1, self.vocab_size), Des[1:].contiguous().view(-1))
        return loss

    def generate(self, img1, img2, beam_size = 1):
        # Img_G, Imgs = self.encode_img(img1.unsqueeze(0), img2.unsqueeze(0))
        Imgs = self.encode_img(img1.unsqueeze(0), img2.unsqueeze(0))
        Des = Variable(torch.ones(1, 1).long()).cuda()
    
        Des = self.beam_search(Imgs, beam_size)
        return Des.squeeze()
    ...
    def beam_search(self, Imgs, beam_size):
        LENGTH_NORM = True
        ...
        for i in range(self.max_len-1):
            if i == 0:
                ...
                out = self.decode(Des, Imgs.repeat(beam_size, 1, 1), Variable(subsequent_mask(Des.size(0), Des.size(1))).cuda())
                # print('Out ', out.size())
